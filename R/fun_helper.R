...
FindPseudoBulkMarkers <- function(seurat_object){
  
  counts    <- seurat_object@assays$RNA@counts
  meta.data <- seurat_object@meta.data
  sce       <- SingleCellExperiment::SingleCellExperiment(assays = list(counts = counts), colData = meta.data)
  
  # aggregate counts for each pseudosample (patient/clonotype/condition)
  groups      <- colData(sce)[,c("orig.ident")]
  aggr_counts <- Matrix.utils::aggregate.Matrix(t(counts(sce)), groupings = groups, fun = "sum")
  aggr_counts <- t(aggr_counts)
  
  ## massage metadata
  meta.data    <- colData(sce) %>% as.data.frame() %>% dplyr::select(project, orig.ident)
  meta.data    <- meta.data[!duplicated(meta.data),]
  rownames(meta.data) <- meta.data$orig.ident
  sample_order <- match(colnames(aggr_counts), meta.data$orig.ident)
  meta.data    <- meta.data[sample_order,]
  
  #perform DEG analysis for pseudobulked data
  # Create DESeq2 object        
  dds <- DESeqDataSetFromMatrix(countData = aggr_counts, colData = meta.data, design = ~ project)
  dds$project <- factor(as.character(dds$project), levels = c("AA BM", "Healthy BM"))
  dds_results <- DESeq(dds)
  dds_results
  
  res <- results(dds_results, alpha = 0.05) 
  res$log2FoldChange <- -res$log2FoldChange
  res <- res %>% as.data.frame() %>% add_rownames(var = "gene")
  return(res)
